...
* create file by executing the following Bash command(s)
```sh
...
```
* append the following to the *list-tables* file
...
aws dynamodb list-tables $ENDPOINT_URL \
  ...
  --output table
```
* make the script user executable
```sh
...
```
* append the following to the *delete-table* file
...
```
* make the script executable by user
```sh
chmod u+x backend-flask/bin/DDB/delete-table
```
* execute script to see if it completes successfully
```sh
...
```
* append the following to the *seed* file
...
def create_message(client=dynamodb,
    ...
    pass
    # TODO: users is a list of dict


def get_user_uuids() -> dict:
    sql = """
      SELECT
          users.uuid,
          users.display_name,
      ...
      FROM users
      WHERE
          users.handle IN (%(user_1)s, %(user_2)s)
    """

    users: list[dict[str, any]] = db.query_array_json(sql, {'user_1': 'the_captain', 'user_2': 'Blaviken_Butcher'})
    user_1: dict[str, any] = next((user for user in users if user['handle'] == 'the_captain'), None)
    user_2: dict[str, any] = next((user for user in users if user['handle'] == 'Blaviken_Butcher'), None)
    results: dict[str, any] = {'user_1': user_1, 'user_2': user_2}
    print(results)
    return results
...
def create_message_group(
                         ...
                         other_user_uuid=users['user_2']['uuid'],
                         my_user_uuid=users['user_1']['uuid'],
                         other_user_display_name=users['user_2']['display_name'],
                         other_user_handle=users['user_2']['handle'],
                         last_message_at=now.isoformat(),
                         message='this is a filler message'
                         ):
    table_name: str = 'cruddur-messages'
    record: dict[str, any] = {
      'primary_key': {'S': f"GRP#{my_user_uuid}"},
...
# appends parent path to sys environment to faciliate importing/navigating to other parent directories
sys.path.append(parent_path)
from lib.db import db  # adjustments needs to be made to import path
...
# mock conversation to seed DynamoDB table
conversation = """
...
message_group_uuid = '78211121-0f4d-4c2d-9186-a1162d4ac74e'  # str(uuid.uuid4())
now = datetime.now(timezone.utc).astimezone()
users = get_user_uuids()

# message group from user_1's perspective
...
# message group from user_2's perspective
create_message_group(
                     client=dynamodb,
                     message_group_uuid=message_group_uuid,
                     my_user_uuid=users['user_2']['uuid'],
                     other_user_uuid=users['user_1']['uuid'],
                     other_user_display_name=users['user_1']['display_name'],
                     other_user_handle=users['user_1']['handle'],
                     last_message_at=now.isoformat(),
                     message='this is a filler message'
                     )

# removes empty space at the beginning and end of string,
# then splits the string at the '\n' character into a list of strings.
lines = conversation.lstrip().rstrip().split('\n')
...
for index, line in enumerate(lines):
  ...
  created_at: tuple = (now + timedelta(minutes=index)).isoformat()
  create_message(
                 client=dynamodb,
                 message_group=message_group_uuid,
                 created_at=created_at,
